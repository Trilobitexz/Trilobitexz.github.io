...
interface TaiwanTopBottomStocksProps {
...
export default function TaiwanTopBottomStocks({
  onSelectStock,
  onAddToWatchlist,
  watchlistSymbols,
  colorConvention,
}: TaiwanTopBottomStocksProps) {
  ...
  return (
    <div className="bg-[#161B22] border border-[#30363D] rounded-xl p-5 space-y-4 shadow-lg h-full relative" id="taiwan-top-bottom">
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-[#30363D] pb-3">
        <div className="flex items-center gap-2">
          <div className="p-1 px-1.5 rounded bg-[#58A6FF]/10 border border-[#58A6FF]/20">
          ...
          <div>
            ...
            <p className="text-[10px] text-zinc-500">
              即時追蹤上市櫃權值與熱門個股，查看當日漲跌幅表現兩極端的熱門標的
            </p>
          </div>
        </div>
        <button
          ...
          disabled={isLoading}
          className="flex items-center gap-1.5 text-[#8B949E] hover:text-[#C9D1D9] text-[10px] uppercase font-bold tracking-wider transition-colors disabled:opacity-50"
        >
              ...
              <table className="w-full text-left text-xs text-[#C9D1D9] whitespace-nowrap">
                <thead className="sticky top-0 bg-[#0D1117] border-b border-[#30363D] z-10">
                  <tr className="text-[10px] text-zinc-500 font-mono uppercase">
                    ...
                    <th className="px-3 py-2 font-medium">代碼與簡稱</th>
                    ...
                    return (
                      <tr key={stock.symbol} className="hover:bg-[#161B22] transition-colors group">
                          ...
                          <div className="flex items-center gap-2">
                            <div className="w-12 shrink-0">
                              <button
                                ...
                                type="button"
                                className="font-mono text-[10px] font-bold text-[#58A6FF] hover:underline bg-[#58A6FF]/5 hover:bg-[#58A6FF]/10 px-1 py-0.5 rounded cursor-pointer w-full text-left truncate"
                                title={stock.symbol.replace(".TW", "")}
                              >
                            ...
                            <div className="flex-1 min-w-0 flex items-center gap-1.5">
                              <span 
                                onClick={() => onSelectStock(stock.symbol)}
                                className="font-bold text-zinc-200 group-hover:text-[#58A6FF] cursor-pointer transition-colors text-[11px] leading-tight block truncate"
                                title={stock.name}
                              >
                                {stock.name.split(' ')[0]}
                              </span>
                              {stock.symbol.startsWith("00") && (
                                <span className="bg-purple-500/10 text-purple-400 border border-purple-500/20 text-[9px] leading-none px-1 py-0.5 rounded font-bold shrink-0">
                                  ETF
                                </span>
                              )}
                            </div>
                        ...
                        <td className="px-3 py-2.5 text-right font-mono text-zinc-400">
                          {stock.volume ? Math.round(stock.volume / 1000).toLocaleString() : '-'}
                        </td>
