...
// Find the line index containing "AI評分給予" which was corrupted (should be around 1317)
let startIndex = -1;
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('rationality = `AI評分給予 ${score.toFapp.post')) {
    startIndex = i;
    break;
  }
}

// Find the line index of createFallbackSparkline
let endIndex = -1;
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('function createFallbackSparkline(')) {
    endIndex = i;
    break;
  }
}
